...
@pytest.mark.django_db
class TestVerifyParticipants:
    """Guard rail for the Team -> MatchParticipant migration."""

    def test_clean_database_reports_ok(self):
        from .conftest import ScheduledMatchFactory
        ...
        out = StringIO()
        call_command("verify_participants", stdout=out)

        assert "OK" in out.getvalue()

    def test_missing_participant_row_is_reported(self):
        from pingpong.models import MatchParticipant

        m = MatchFactory()
        MatchParticipant.objects.filter(match=m).first().delete()

        out = StringIO()
        with pytest.raises(SystemExit):
            call_command("verify_participants", stdout=out)
        assert "side" in out.getvalue()

    def test_wrong_winner_side_is_reported(self):
        m = MatchFactory()
        for n in (1, 2, 3):
        ...
        Match.all_objects.filter(pk=m.pk).update(winner_side=2)

        out = StringIO()
        with pytest.raises(SystemExit):
            call_command("verify_participants", stdout=out)
        assert "winner_side" in out.getvalue()
