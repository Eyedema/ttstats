...
class TestSendMatchConfirmationEmail:
    ...
    def test_win_result_player1(self):
        ...
        mail.outbox.clear()
        send_match_confirmation_email(m, m.player1, m.player2)
    ...
    def test_loss_result_player2(self):
        m = self._make_complete_match()
        mail.outbox.clear()
        send_match_confirmation_email(m, m.player2, m.player1)
        assert len(mail.outbox) == 1
...
class TestSendScheduledMatchEmail:
    def test_email_sent(self):
        ...
        mail.outbox.clear()
        send_scheduled_match_email(sm, sm.player1, sm.player2)
        assert len(mail.outbox) == 1
    ...
    def test_no_user_early_return(self):
        ...
        mail.outbox.clear()
        send_scheduled_match_email(sm, p1, p2)
    ...
    def test_no_email_early_return(self):
        ...
        mail.outbox.clear()
        send_scheduled_match_email(sm, u, p2)
        assert len(mail.outbox) == 0
